...
function Demo() {
  document.addEventListener(
    ...
    function() {
      ...
      this.updateUserButton = document.getElementById('demo-update-button');
      ...
      this.updateUserUrl = window.location.href + 'update_user_info';
      ...
      this.updateUserButton.addEventListener(
        'click',
        this.startUpdateRequest.bind(this)
      );
...
// Does an authenticated request to a Firebase Functions endpoint using a __session cookie.
Demo.prototype.startUpdateRequest = function() {
  // Set the __session cookie.
  firebase
    .auth()
    .currentUser.getIdToken(true)
    .then(
      function(token) {
        ...
        req.open('POST', this.updateUserUrl, true);
        req.send(
          JSON.stringify({ user: { fullname: 'some full name', age: 10 } })
        );
      }.bind(this)
    );
};
